...
function EditAppoinment()
{
    ...
    useEffect(()=>{
        Axios.get("https://wellnesswise-backend-a19c.onrender.com/doctorRoute/update/" + id)
        .then((res)=>{
            if(res.status === 200){
    ...
    const handleSubmit = () => {
        ...
        Axios.put("https://wellnesswise-backend-a19c.onrender.com/doctorRoute/update/" + id , data)
        .then((res)=>{
            if(res.status === 200){
